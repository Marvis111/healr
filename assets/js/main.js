(function ($) {
  ...
  if (typeof $.fn.mCustomScrollbar === "function") {
    $("[data-scroll-options]").each(function () {
      var _this = $(this);
      var scrollOptions = _this.data("scroll-options") || '{"theme": "light"}';
      try {
  ...
  // Location-based hero image function
  function updateHeroImageBasedOnLocation() {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        function(position) {
          ...
          // Define regions in Nigeria with their coordinates
          ...
          let userRegion = null;
          for (const [region, data] of Object.entries(nigeriaRegions)) {
          ...
          // Update hero image based on region
          const heroImage = document.querySelector('.hero-image');
          if (heroImage) {
            let imagePath = 'assets/img/hero-1.webp'; // Default image
            
            if (userRegion) {
              imagePath = `assets/img/hero-${userRegion}.webp`;
            }
            
            // Check if the region-specific image exists
            const img = new Image();
            img.onload = function() {
  ...
  // Initialize location-based hero image when page loads
  $(document).ready(function() {
    updateHeroImageBasedOnLocation();
  });
    ...
    menuLinks.each(function () {
      ...
      if (hasPos > -1) {
        ...
        if (res != "" && res != "#" && $(res).length) {
          ...
          // Check if the element exists and has a valid position
          if (refElement.length && refElement.position() && refElement.position().top !== undefined) {
            if (
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        function(position) {
          ...
          const heroImage = document.querySelector('.hero-image');
          if (heroImage) {
            let imagePath = 'assets/img/hero-1.webp';             
            if (userRegion) {
              imagePath = `assets/img/hero-${userRegion}.webp`;
            }
